...
// Get medical history (appointments, prescriptions, reports)
$stmt = $pdo->prepare("SELECT 'appointment' as type, a.id, a.appointment_date as date, 
                      u.name as doctor_name, d.specialty, NULL as title
                      FROM appointments a
                      ...
                      SELECT 'prescription' as type, p.id, p.created_at as date,
                      u.name as doctor_name, d.specialty, NULL as title
                      ...
                      SELECT 'report' as type, mr.id, mr.uploaded_at as date,
                      u.name as doctor_name, d.specialty, mr.report_name as title
...
<head>
    ...
    <title>Medical History - E Care Hub</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="nav flex-column mt-4">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="medicalhistory.php">
    ...
    <div class="container-fluid py-4">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800 font-weight-bold">Medical History</h1>
        </div>
        ...
        <?php if (empty($medicalHistory)): ?>
            <div class="card shadow">
                <div class="empty-state">
                    <i class="fas fa-history"></i>
                    <h4 class="text-muted">No medical history</h4>
                    <p class="text-muted">Your medical history will appear here after consultations</p>
                </div>
            </div>
        <?php else: ?>
            <div class="card shadow">
                <div class="card-body">
                    <div class="timeline">
                        <?php foreach ($medicalHistory as $item): ?>
                            <div class="timeline-item">
                                ...
                                <div class="timeline-content history-card mb-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h5 class="mb-0">
                                            ...
                                            <?php elseif ($item['type'] === 'prescription'): ?>
                                                Prescription
                                            <?php else: ?>
                                                Medical Report: <?= htmlspecialchars($item['title']) ?>
                                            <?php endif; ?>
                                        </h5>
                                    ...
                                    <div class="d-flex align-items-center mb-3">
                                        <div class="mr-3">
                                            <div class="user-avatar" style="width: 40px; height: 40px; font-size: 1rem;">
                                                <?= strtoupper(substr($item['doctor_name'], 0, 1)) ?>
                                            </div>
                                        </div>
                                        <div>
                                            <p class="mb-0 font-weight-bold">Dr. <?= htmlspecialchars($item['doctor_name']) ?></p>
                                            <p class="mb-0 text-muted small"><?= htmlspecialchars($item['specialty']) ?></p>
                                        </div>
                                    </div>
                                    
                                    <div class="d-flex justify-content-end">
                                        <?php if ($item['type'] === 'appointment'): ?>
                                            <a href="appointmentdetails.php?id=<?= $item['id'] ?>" class="btn btn-sm btn-primary">
                                                <i class="fas fa-eye mr-1"></i> View Details
                                            </a>
                                        <?php elseif ($item['type'] === 'prescription'): ?>
                                            <a href="prescriptiondetails.php?id=<?= $item['id'] ?>" class="btn btn-sm btn-primary">
                                                <i class="fas fa-eye mr-1"></i> View Prescription
                                            </a>
                                        <?php else: ?>
                                            <a href="reportviewer.php?id=<?= $item['id'] ?>" class="btn btn-sm btn-primary">
